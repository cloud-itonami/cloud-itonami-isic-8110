(ns facilitiesops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (FacilitiesSupportAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the FacilitiesSupportGovernor
  ...
  human facility-operations coordinator. The approver resumes with
...
(defn build
  ...
  `facilitiesops.store/Store`).
  opts:
    :advisor      -- a `facilitiesops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FacilitiesSupportAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; FacilitiesSupportGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
