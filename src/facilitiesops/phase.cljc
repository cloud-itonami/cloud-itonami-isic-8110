(ns facilitiesops.phase
  "Phase 0->3 staged rollout for the ISIC-8110 'Combined facilities
  support activities' operations-coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- service-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds combined-service crew
  ...
  `:flag-facility-concern` is deliberately ABSENT from every phase's
  `:auto` set, including phase 3 -- a permanent structural fact, not a
  rollout milestone still to come. Flagging a security-incident/
  maintenance-hazard/access-control concern always needs a human to
  actually look at it. `facilitiesops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently -- two
  layers, not one, agree on this."
  (:require [facilitiesops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-facility-concern` is a member of
...
(defn gate
  ...
  - `:flag-facility-concern` is never auto-eligible at any phase, so it
...
(defn verdict->disposition
  "Map a FacilitiesSupportGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
