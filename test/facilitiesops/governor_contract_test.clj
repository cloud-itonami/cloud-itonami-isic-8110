(ns facilitiesops.governor-contract-test
  ...
  (testing "clean service-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-service-record :facility-id "facility-1" :patch {:rounds-completed 4}}
                               ctx)]
      (is (some? result))
...
(deftest facility-concern-always-escalates
  (testing ":flag-facility-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-2" :phase 3}
          result (exec-request actor "t2"
                               {:op :flag-facility-concern :facility-id "facility-1"
                                :patch {:concern "tailgating observed" :confidence 0.99}}
                               ctx)]
      (is (some? result))
      ;; At this point the actor is paused for approval, not yet committed
      (is (= 0 (count (store/coordination-log db)))
          "facility concern must not auto-commit, must wait for approval")
      ;; Now approve it
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :facility-id "facility-1"
                                :patch {:item "HVAC filter replacement bulk order" :quantity 1 :estimated-cost 4200.0
  ...
  (testing "unregistered facility -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t3"
                     {:op :log-service-record :facility-id "unknown-facility"
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into access-credential/emergency-response scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8b"
                     {:op :log-service-record :facility-id "unknown" :patch {:rounds-completed 4}}
                     ctx)
